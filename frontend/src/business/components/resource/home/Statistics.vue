  ...
  <main-container v-loading="result.loading">
    ...
    <el-card class="table-card" v-if="groups.length > 0">
      <el-tabs class="border-card" type="border-card" @tab-click="handleClick">
        <el-tab-pane
          ...
          <el-row>
            <el-col :span="12">
              <label-value :label="$t('resource.scene_name')">{{ group.name }}</label-value>
            </el-col>
            <el-col :span="12">
              <label-value :label="$t('resource.activation_time')">{{
                  group.createTime | timestampFormatDate }}
              </label-value>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="12">
              <label-value :label="$t('resource.scene_description')">{{ group.description }}</label-value>
            </el-col>
            <el-col :span="12">
              <label-value :label="$t('resource.activation_time')">{{ group.state }}</label-value>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="12">
              <label-value :label="$t('resource.equal_guarantee_level')">{{ group.level }}</label-value>
            </el-col>
            <el-col :span="12">
              <label-value :label="$t('resource.compliance_results')">
                <span v-if="group.status === 'risky'" style="color: red;">
                  <i class="el-icon-warning"/>
                  {{ $t('resource.discover_risk') }}
                </span>
                <span v-if="group.status === 'risk_free'" style="color: green;">
                  <i class="el-icon-warning"/>
                  {{ $t('resource.no_risk') }}
                </span>
              </label-value>
    ...
    <el-card>
      ...
      <el-table border :data="tableData" class="adjust-table table-content" @sort-change="sort" @filter-change="filter"
        ...
        <el-table-column prop="itemSortFirstLevel" :label="$t('resource.security_level')" min-width="10%"
                         show-overflow-tooltip></el-table-column>
        <el-table-column prop="itemSortSecondLevel" :label="$t('resource.control_point')" min-width="10%"
                         show-overflow-tooltip></el-table-column>
        <el-table-column prop="project" :label="$t('resource.basic_requirements_for_grade_protection')" min-width="47%"
                         show-overflow-tooltip></el-table-column>
        <el-table-column :label="$t('resource.pre_check_results')" min-width="10%" show-overflow-tooltip>
          <template v-slot:default="scope">
            ...
            <el-tooltip class="item" effect="dark" :content="$t('resource.requirements_of_the_regulations')"
                        placement="top">
        ...
        <el-table-column v-slot:default="scope" :label="$t('resource.suggestions_for_improvement')" min-width="10%">
          <el-tooltip v-if="scope.row.status === 'risky'" class="item" effect="dark" :content="scope.row.improvement"
                      placement="top">
    ...
    <!--Rule detail-->
    <el-drawer class="rtl" :title="$t('resource.report_detail')" :visible.sync="visible" size="60%"
               :before-close="handleClose" :direction="direction"
               :destroy-on-close="true">
      <el-row class="el-row-c">
        <el-col :span="8"><span
          style="color: #909090;">{{ $t('resource.basic_requirements_for_grade_protection') }}</span></el-col>
      ...
      <el-row class="el-row-c">
        <el-col :span="8"><span style="color: #909090;">{{ $t('resource.i18n_detail') }} <i
          class="el-icon-question"></i></span></el-col>
        <el-col :span="16">
              <span>
                <el-table :data="detailForm.taskList" style="width: 100%">
                    <el-table-column :label="$t('rule.rule_name')" min-width="75%">
                      <template slot-scope="scope">
                        <span v-if="!!scope.row.returnSum && scope.row.returnSum>0"><i class="el-icon-warning-outline"
                                                                                       style="color: red"></i> {{
                            scope.row.taskName }}</span>
                        <span v-else><i class="el-icon-circle-check"
                                        style="color: #00bb00"></i> {{ scope.row.taskName }}</span>
                      </template>
                    </el-table-column>
                    <el-table-column prop="returnSum" :label="$t('resource.risk_point')" min-width="25%">
                      <template slot-scope="scope">
                        <el-link v-if="!!scope.row.returnSum && scope.row.returnSum>0" style="color: red"
                                 @click="innerDrawerOpen(scope.row)"> {{
                            scope.row.returnSum ? scope.row.returnSum : 0
                          }} / {{ scope.row.resourcesSum ? scope.row.resourcesSum : 0 }}</el-link>
                        <span
                          v-else> {{
                            scope.row.returnSum ? scope.row.returnSum : 0
                          }} / {{ scope.row.resourcesSum ? scope.row.resourcesSum : 0 }}</span>
                      </template>
                    </el-table-column>
                </el-table>
              </span>
      ...
      <el-row class="el-row-c">
        <el-col :span="8"><span style="color: #909090;">{{ $t('resource.suggestions_for_improvement') }} <i
          class="el-icon-question"></i></span></el-col>
      ...
      <el-drawer
        ...
        <el-form :model="ruleForm" label-position="right" label-width="120px" size="small" ref="ruleForm">
          ...
          <el-form-item :label="$t('account.cloud_platform')">
            <el-select style="width: 100%;" v-model="ruleForm.pluginId"
                       :placeholder="$t('account.please_choose_plugin')">
              <el-option
                v-for="item in plugins"
                :key="item.id"
                :label="item.name"
                :value="item.id">
                <img :src="require(`@/assets/img/platform/${item.icon}`)"
          ...
          <el-form-item :label="$t('rule.rule_tag')">
            <el-select style="width: 100%;" multiple v-model="ruleForm.tags"
                       :placeholder="$t('rule.please_choose_tag')">
              <el-option
                v-for="item in tags"
                :key="item.tagKey"
                :label="item.tagName"
          ...
          <el-form-item :label="$t('rule.severity')">
            <el-select style="width: 100%;" v-model="ruleForm.severity"
                       :placeholder="$t('rule.please_choose_severity')">
              <el-option
