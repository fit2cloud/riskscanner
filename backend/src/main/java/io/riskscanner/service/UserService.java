...
import io.riskscanner.security.RsUserToken;
...
import static io.riskscanner.commons.constants.SessionConstants.ATTR_USER;
...
public class UserService {
    ...
    public UserDTO insert(UserRequest user) throws Exception {
        ...
        if (user1 != null) {
            RSException.throwException(Translator.get("user_id_already_exists"));
        } else {
            createUser(user);
        }
    ...
    private void checkUserParam(User user) {
        ...
        if (StringUtils.isBlank(user.getId())) {
            RSException.throwException(Translator.get("user_id_is_null"));
        }
    ...
    private void checkEmailIsExist(String email) {
        ...
        if (!CollectionUtils.isEmpty(userList)) {
            RSException.throwException(Translator.get("user_email_already_exists"));
        }
    }
    ...
    public List<User> getUserListWithRequest(io.riskscanner.controller.request.UserRequest request) {
    ...
    public boolean checkUserPassword(String userId, String password) throws Exception {
        ...
        if (StringUtils.isBlank(password)) {
            RSException.throwException(Translator.get("password_is_null"));
        }
        UserExample example = new UserExample();
