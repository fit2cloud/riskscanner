...
public class SystemParameterService {
    ...
    public void testWechatConnection(Map<String, String> hashMap) {
        ...
        if (responseEntity.getStatusCodeValue() != 200) {
            RSException.throwException("request failed.");
        }
        String body = responseEntity.getBody();
        JSONObject jsonObject = JSONObject.parseObject(body);
        if (jsonObject.getIntValue("errcode") != 0) {
            RSException.throwException(jsonObject.getString("errmsg"));
        }
        ...
        try {
            ...
            RSException.throwException("agentId is not valid.");
        }
        ...
        if (responseResponseEntity.getBody().getErrcode() != 0) {
            RSException.throwException(responseResponseEntity.getBody().getErrmsg());
        }
    }
    ...
    public void testDingtalkConnection(Map<String, String> hashMap) throws Exception {
        ...
        if (response.getErrcode() != 0) {
            RSException.throwException(response.getErrmsg());
        }
        ...
        if (execute.getErrcode() != 0) {
            RSException.throwException(execute.getErrmsg());
        }
        ...
        if (execute1.getErrcode() != 0) {
            RSException.throwException(execute1.getErrmsg());
        }
    }
